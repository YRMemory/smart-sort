...
const long waitTimeout = 15000; // Тайм-аут ожидания 15 сек
...
void loop() {
  // 1. Ждем ЛЮБУЮ команду от C#, чтобы начать слежение
...
void waitForObject() {
  ...
  // Если за 15 секунд ничего не произошло
  Serial.println("ID:TIMEOUT");
}

// Утилита для проверки конкретного датчика
bool checkSensor(int trig, int echo) {
  long distance = getDistance(trig, echo);
  // Если дистанция меньше порога (но не 0)
...
long getDistance(int trig, int echo) {
  ...
  long duration = pulseIn(echo, HIGH, 30000); // 30мс тайм-аут
